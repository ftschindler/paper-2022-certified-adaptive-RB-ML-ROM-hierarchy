...
if config.HAVE_TORCH:
    ...
    def multiple_restarts_training(training_data, validation_data, neural_network,
        ...
        logger_nn = TensorBoardLogger(settings['tb_path'] + date.today().strftime('%d.%m.%y') + '/',
                                      name='model_NN')
        ...
        trainer = pl.Trainer(max_epochs=settings['num_epochs_nn'],
                             ...
                             gpus=settings['gpus'],
                             callbacks=[EarlyStopping(monitor="val/loss", patience=settings['early_stopping_patience'])])

        training_dataset = CustomDataset(training_data)
        validation_dataset = CustomDataset(validation_data)
        train_loader = utils.data.DataLoader(training_dataset, batch_size=settings['batch_size'],
                                             num_workers=settings['num_workers'], shuffle=False)
        val_loader = utils.data.DataLoader(validation_dataset, batch_size=settings['batch_size'],
                                           num_workers=settings['num_workers'], shuffle=False)

        import time
        t0_start = time.time()
        trainer.fit(neural_network, train_loader, val_loader)
        t0_stop = time.time()
        logger.info(f"Overall time required for training: {t0_stop-t0_start} ...")
        logger.info(f"Finished training with training loss of {trainer.callback_metrics['train/loss']} "
                    f"and validation loss of {trainer.callback_metrics['val/loss']} ...")
